import { IAMClient, GetUserCommand, ListRolesCommand, IAMClientConfig, Role } from "@aws-sdk/client-iam";
 ...
 */
export async function getUserName(config: IAMClientConfig) {
...
export async function importListRoles(config: IAMClientConfig, userName: string): Promise<string[]> {
  const client = new IAMClient(config);
