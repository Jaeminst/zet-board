...
export default function DatabaseSearch({ disabled }: { disabled?: boolean }) {
  ...
  function handleSearch(value: string) {
    ...
    const filteredResult: Database[] = databaseList.flatMap(db => {
      // 기존 속성 검사
      const matchInDb =
        db.Identifier.toLowerCase().includes(search) ||
      ...
      const settingsMatch =
        db.Endpoint.Address &&
        databaseSettingList[db.Endpoint.Address] &&
      ...
      if ('Instances' in db && 'ReaderEndpoint' in db) {
        // db가 DbCluster 타입임을 확인
        const matchedInstances = db.Instances.filter(instance => {
          const instanceMatch =
            instance.Identifier.toLowerCase().includes(search) ||
            instance.Engine.toLowerCase().includes(search) ||
            instance.Size.toString().toLowerCase().includes(search);

          const instanceSettingsMatch =
            instance.Endpoint.Address &&
            databaseSettingList[instance.Endpoint.Address] &&
