...
const DatabaseTableRow = ({
  ...
  const updateDatabaseField = useCallback(
    (endpoint: Endpoint, field: keyof DatabaseSetting[string], value: string | boolean) => {
    ...
    <>
      <TableRow key={database.Identifier}>
        <TableCell className="w-[80px] h-[50px] px-4 pt-1 pb-1">
          <Switch
            checked={
              database.Endpoint.Address ? databaseSettingList?.[database.Endpoint.Address]?.tunneling ?? false : false
            }
        ...
        <TableCell className="w-[120px] p-1 whitespace-nowrap">
          {database.Role == 'Cluster' ? `${database.Size} 인스턴스` : database.Size}
        </TableCell>
      </TableRow>
      {database.Role == 'Cluster' && isExpanded && (
        <InstanceRows instances={database.Instances} updateDatabaseField={updateDatabaseField} />
      )}
    </>
  );
};
...
const InstanceRows = ({
  ...
  instances: Database[];
  updateDatabaseField: (endpoint: Endpoint, field: keyof DatabaseSetting[string], value: string | boolean) => void;
}) => {
      ...
      {instances.map(instance => (
        <TableRow key={instance.Identifier}>
          <TableCell className="w-[80px] h-[50px] px-4 pt-1 pb-1">
            <Switch
              checked={
                instance.Endpoint.Address ? databaseSettingList?.[instance.Endpoint.Address]?.tunneling ?? false : false
              }
          ...
          <TableCell className="w-[80px] p-1"></TableCell>
          <TableCell className="w-[80px] p-1">
            <EditableField
              label="LocalPort"
